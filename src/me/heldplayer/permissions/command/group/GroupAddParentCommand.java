...
public class GroupAddParentCommand extends AbstractSubCommand {
    ...
    public void runCommand(CommandSender sender, String alias, String... args) {
        ...
        if (parentGroup.doesInheritFrom(permissions)) {
            sender.sendMessage(Permissions.format("Group '%s' is already a parent of '%s'", ChatColor.RED, group, parent));
            return;
        }

        permissions.addParent(parentGroup);
        sender.sendMessage(Permissions.format("Made '%s' a child of '%s'", ChatColor.GREEN, group, parent));
    ...
    public List<String> getTabCompleteResults(CommandSender sender, String alias, String... args) {
        ...
        if (args.length == 2) {
            ...
            return TabHelper.tabAnyGroupExcept(permissions.getAllGroupNames(), permissions.name);
        }
